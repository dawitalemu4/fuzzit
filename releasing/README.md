...
This document provides instructions for building a release of `fuzzit`.

The release process consists of a handful of tasks:
1. Drop a release tag in git.
2. Build binaries for various platforms.
3. Creates a release in GitHub, uploads the binaries, and creates provisional release notes (in the form of a change log).
4. Submits a PR to update the [Homebrew](https://brew.sh) recipe with the latest version.

Most of this is automated via a script in this same directory. The main thing you will need is a GitHub personal access token, which will be used for creating the release in GitHub (so you need write access to the dawitalemu4/fuzzit repo) and to open a Homebrew pull request.
...
GITHUB_TOKEN=abcdef0123456789abcd ./releasing/release.sh v2.3.4
```
...
The GitHub release is the first step performed by the `release.sh` script. So generally, if there is an issue with that step, you can re-try the whole script.
...
# from the root of the repo
git tag v2.3.4
GITHUB_TOKEN=abcdef0123456789abcdef make release
```
...
URL=https://github.com/dawitalemu4/fuzzit/archive/refs/tags/v2.3.4.tar.gz
...
```sh
HOMEBREW_GITHUB_API_TOKEN=abcdef0123456789abcdef brew bump-formula-pr --url $URL --sha256 $SHA fuzzit 
```
...
_Credit to https://github.com/fullstorydev/grpcurl for the releasing template._
